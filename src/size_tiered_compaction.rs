...
impl ConflictChecker {
    fn new(compactions: &Vec<Compaction>) -> Self {
    ...
    fn add_compaction(&mut self, compaction: &Compaction) {
        ...
        self.sources_used.insert(SourceId::SortedRun(compaction.destination));
    }
}
...
impl BackpressureChecker {
    ...
    fn check_compaction(
        ...
        let estimated_result_size: u64 = sources.iter()
            .map(|src| src.size)
            .sum();
...
impl CompactionScheduler for SizeTieredCompactionScheduler {
    fn maybe_schedule_compaction(&self, state: &CompactorState) -> Vec<Compaction> {
        ...
        while state.compactions().len() + compactions.len() < 4 {
            let Some(compaction) = self.pick_next_compaction(
                &l0,
                &srs,
                &checker
            ) else {
...
impl SizeTieredCompactionScheduler{
    ...
    fn pick_next_compaction(
        ...
        let l0_candidate: VecDeque<_> = l0.iter().cloned().collect();
        if let Some(mut l0_candidate) = self.clamp_min(l0_candidate) {
            l0_candidate = self.clamp_max(l0_candidate);
            let dst = srs.first()
                .map_or(0, |sr| sr.source.unwrap_sorted_run() + 1);
            let next_sr = srs.first();
            if checker.check_compaction(&l0_candidate, dst, next_sr) {
                return Some(self.create_compaction(l0_candidate, dst));
            }
        }

        // try to compact the lower levels
        let mut srs_iter = srs.iter().peekable();
        while let Some(_) = srs_iter.peek() {
    ...
    fn create_compaction(&self, sources: VecDeque<CompactionSource>, dst: u32) -> Compaction {
        let sources: Vec<SourceId> = sources.iter()
    ...
    fn compaction_sources(&self, db_state: &CoreDbState) -> (Vec<CompactionSource>, Vec<CompactionSource>) {
        (
            db_state.l0.iter()
...
mod tests {
    ...
    fn test_should_compact_l0s_to_first_sr() {
        // given:
        let scheduler = SizeTieredCompactionScheduler::new(SizeTieredCompactionSchedulerOptions::default());
        let l0 = vec![create_sst(1), create_sst(1), create_sst(1), create_sst(1)];
        let state = create_compactor_state(
            create_db_state(
        ...
        let compaction = compactions.get(0).unwrap();
    ...
    fn test_should_not_compact_l0s_if_fewer_than_min_threshold() {
        // given:
        let scheduler = SizeTieredCompactionScheduler::new(SizeTieredCompactionSchedulerOptions::default());
        let l0 = vec![create_sst(1), create_sst(1), create_sst(1)];
        let state = create_compactor_state(
            create_db_state(
                l0.iter().cloned().collect(),
                vec![],
    ...
    fn test_should_compact_srs_if_enough_with_similar_size() {
        ...
        let state = create_compactor_state(
            create_db_state(
                VecDeque::new(),
                vec![create_sr2(4, 2), create_sr2(3, 2), create_sr2(2, 2), create_sr2(1, 2), create_sr2(0, 2)],
            )
        );
        ...
        assert_eq!(compactions.len(), 1);
        let compaction = compactions.get(0).unwrap();
        assert_eq!(compaction.clone(), create_sr_compaction(vec![4, 3, 2, 1, 0]))
    }
    ...
    fn test_should_only_include_srs_if_with_similar_size() {
        ...
        let state = create_compactor_state(
            create_db_state(
                VecDeque::new(),
                vec![create_sr2(4, 2), create_sr2(3, 2), create_sr2(2, 2), create_sr2(1, 2), create_sr2(0, 10)],
            )
        );
    ...
    fn test_should_not_schedule_compaction_for_source_that_is_already_compacting() {
        // given:
        let scheduler = SizeTieredCompactionScheduler::new(SizeTieredCompactionSchedulerOptions::default());
        let mut state = create_compactor_state(
            create_db_state(
                VecDeque::new(),
                vec![create_sr2(4, 2), create_sr2(3, 2), create_sr2(2, 2), create_sr2(1, 2), create_sr2(0, 2)],
            )
        );
        state.submit_compaction(create_sr_compaction(vec![3, 2, 1, 0])).unwrap();
    ...
    fn test_should_not_compact_srs_if_fewer_than_min_threshold() {
        ...
        let state = create_compactor_state(
            create_db_state(
                VecDeque::new(),
                vec![
                    create_sr2(2, 2), create_sr2(1, 2), create_sr4(0, 2)
                ],
            )
        );
    ...
    fn test_should_clamp_compaction_size() {
        ...
        let state = create_compactor_state(
            create_db_state(
                VecDeque::new(),
                vec![
                    create_sr2(11, 2), create_sr2(10, 2), create_sr2(9, 2), create_sr2(8, 2),
                    create_sr2(7, 2), create_sr2(6, 2), create_sr2(5, 2), create_sr2(4, 2),
                    create_sr2(3, 2), create_sr2(2, 2), create_sr2(1, 2), create_sr4(0, 2)
                ],
            )
        );
        ...
        // then:
        assert_eq!(compactions.len(), 1);
        assert_eq!(compactions.get(0).unwrap(), &create_sr_compaction(vec![7, 6, 5, 4, 3, 2, 1, 0]));
    }
    ...
    fn test_should_apply_backpressure() {
        ...
        state.submit_compaction(create_sr_compaction(vec![7, 6, 5, 4, 3, 2, 1, 0])).unwrap();
    ...
    fn test_should_return_multiple_compactions() {
        ...
        let state = create_compactor_state(
            create_db_state(
                l0.iter().cloned().collect(),
                vec![
                    create_sr2(10, 2), create_sr2(9, 2), create_sr2(8, 2), create_sr2(7, 2),
                    create_sr4(3, 16), create_sr4(2, 16), create_sr4(1, 16), create_sr4(0, 16)
                ],
            )
        );
    ...
    fn create_sr(id: u32, sst_size: u64, num_ssts: usize) -> SortedRun {
        let ssts: Vec<SSTableHandle> = (0..num_ssts)
            .map(|_| create_sst(sst_size))
            .collect();
        SortedRun{
            id,
            ssts,
    ...
    fn create_l0_compaction(l0: &Vec<SSTableHandle>, dst: u32) -> Compaction {
        Compaction::new(
            l0.iter().map(|h| SourceId::Sst(h.id.unwrap_compacted_id())).collect(),
            dst
        )
    }
